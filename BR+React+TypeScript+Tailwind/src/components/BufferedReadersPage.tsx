...
const BufferedReadersPage = () => {
    ...
    return (
        <div className="w-full min-h-screen bg-[#FFFEF3] overflow-hidden">
            ...
            {/* Archive Header */}
            <div
                className="w-full pt-20 pb-20 bg-gradient-to-r from-[#FFF8B5] to-[#FED1D8] text-center border-b border-[#ccc] max-[900px]:pt-5 max-[900px]:pb-5 max-[900px]:px-4"
            >
                <div className="flex flex-col items-center max-w-[800px] mx-auto">
                    <h1
                        className="text-[40px] font-bold mb-2.5 max-[900px]:text-[30px]"
                        style={{ fontFamily: "'Inria Serif', serif" }}
                    >
                        BUFFERED READERS ARCHIVE
                    </h1>
            ...
            {/* Magazines Section */}
            {!loading && !error && (
                <div className="py-10 bg-gradient-to-b from-[#FFF4DE] to-[#FFECE6] relative">
                    {Object.entries(pdfData)
                        .filter(([year]) => activeFilter === "ALL" || activeFilter === year)
                        .map(([year, pdfs], index) => (
                            <div
                                key={index}
                                className="mx-auto mb-10 max-w-[90%] bg-[#FDFDDA] rounded-[10px] p-5 shadow-[0_2px_5px_rgba(0,0,0,0.1)] max-[900px]:max-w-[95%]"
                            >
                                <div
                                    className="text-[24px] font-bold mb-5 px-2.5 cursor-pointer"
                                                ...
                                                >
                                                    <div className="flex h-full p-5 max-[900px]:flex-row max-[900px]:p-4">
                                                        <div className="flex-1 flex flex-col justify-start pr-2.5 max-[900px]:justify-center max-[900px]:pr-3">
                                                            ...
                                                            <button
                                                                className="bg-[#F3FB59] border-none rounded-[8px] px-5 py-2 font-medium cursor-pointer mt-2.5 w-fit text-black text-[16px] max-[480px]:px-4 max-[480px]:text-[14px]"
                                                                onClick={() => {
